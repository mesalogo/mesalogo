...
- ❌ **No hard-coded user-visible Chinese (硬编码中文) in frontend `.tsx` source.** Every label / placeholder / button text / `message.*` toast must go through `useTranslation()`. Do not write `t('foo') || '中文 fallback'` either — add the missing key. Use the per-feature namespaces under `frontend/src/locales/<lang>/<ns>.ts`; never recreate the legacy `zh-CN.ts` / `en-US.ts` monolith. See `docs/agents/i18n.md`. Both gates must pass **and you must run them yourself before committing** — there is no automated CI in this repo (see `docs/agents/ci-quality-gates.md`): `cd frontend && pnpm run i18n:check-keys` (zh/en must stay key-consistent) and `pnpm run i18n:check-cjk` (AST-based scanner, zero hard-coded CJK allowed outside `frontend/scripts/i18n-allowlist.json`).
