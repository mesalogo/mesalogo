# /tests/_legacy_snippets/ — Reference snippets archive (参考片段归档)

> History: this directory used to live at repo-root `/tests/` before 2026-05-18.
> In 2026-05 the FastAPI test tree was promoted from `backend-fastapi/tests/` to
> repo-root `/tests/`, and these historical snippets were moved under
> `tests/_legacy_snippets/` so they remain accessible without polluting the
> real test suite.
>
...
> - ⛔ **`pytest` does not collect this directory** (`--ignore=tests/_legacy_snippets` in repo-root `pytest.ini`).
...
> - ✅ Treat them as immutable history. If you need a real test, write it under `tests/<unit|integration|e2e|contract>/` (see `tests/AGENTS.md`).
...
1. ❌ **Do not write new tests here.** New tests go to `tests/<unit\|integration\|e2e\|contract>/`. This dir is closed for new code.
2. ❌ **Do not copy-paste a snippet into `tests/unit|integration|e2e/`.** All snippets are Flask-era / sync-style; they cannot run under FastAPI/async. Rewrite from scratch following the new test conventions.
...
ls tests/_legacy_snippets/ragflow/

# 2. Open it; check the placeholder values it expects.
cat tests/_legacy_snippets/ragflow/test_fixed_ragflow.py
...
RAGFLOW_HOST=192.0.2.50:7080 RAGFLOW_API_KEY=ragflow-... python tests/_legacy_snippets/ragflow/test_fixed_ragflow.py
```

## Historical context

Between 2025-05 and 2025-09 the project ran on Flask without a formal test suite, so engineers piled debug scripts into this folder. After the FastAPI/async migration (2025-10+) the real test directory moved to `backend-fastapi/tests/`. In 2026-05 these snippets were rescued, secret-scrubbed, classified by topic, and committed as immutable reference material rather than thrown away. On 2026-05-18 the FastAPI test tree was promoted to repo-root `/tests/`, and these snippets were moved under `tests/_legacy_snippets/`.
...
_last review: 2026-05-18_ (moved from `/tests/` to `/tests/_legacy_snippets/` during FastAPI test-tree promotion)
